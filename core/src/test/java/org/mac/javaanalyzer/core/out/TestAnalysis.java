...
public class TestAnalysis {
	...
	public void textAnalyserare_randomTextI_variableDefinitionPredicate() throws AnalysisException {
		boolean kommando = TextAnalyserare.VARIABLE_DEFINITION_PATTERN.asPredicate().test("jgjhgfd v = new apa(a,b,c,d,e);");
		Assert.assertThat("Predikat returnerade ej förväntat svar", kommando, IsEqual.equalTo(true) );
	}
	...
	public void textAnalyserare_randomTextK_variableDefinitionPredicate() throws AnalysisException {
		boolean kommando = TextAnalyserare.VARIABLE_DEFINITION_PATTERN.asPredicate().test("jgjhgfd v = new apa(a, b , c, ff, e);");
		Assert.assertThat("Predikat returnerade ej förväntat svar", kommando, IsEqual.equalTo(true) );
	}
	
	@Test
	public void textAnalyserare_randomTextM_variableDefinitionPredicate() throws AnalysisException {
		boolean kommando = TextAnalyserare.VARIABLE_DEFINITION_PATTERN.asPredicate().test("jgjhgfd v = new   apa(a, b , c, ff, e);");
		Assert.assertThat("Predikat returnerade ej förväntat svar", kommando, IsEqual.equalTo(true) );
	}
	...
	public void textAnalyserare_randomTextN_variableDefinitionPredicate() throws AnalysisException {
		boolean kommando = TextAnalyserare.VARIABLE_DEFINITION_PATTERN.asPredicate().test("jgjhgfd v = apa(a, b , c, ff, e);");
		Assert.assertThat("Predikat returnerade ej förväntat svar", kommando, IsEqual.equalTo(false) );
	}@Test
	...
	public void textAnalyserare_identifieraVariabelOchKlass_variableDefinitionPredicate() throws AnalysisException {
		...
		String text = "jgjhgfd v = new apa(a, b , c, ff, e);";
		Kommando kommando = analyserare.parse(text);
		Definition definition = (Definition) kommando;
		Assert.assertThat(String.format("Variabelnamn identifierades inte rätt för '%s'", text), definition.getKey(), IsEqual.equalTo("v"));
		Assert.assertThat(String.format("Klassnamn identifierades inte rätt för '%s'", text), definition.getKey(), IsEqual.equalTo("jgjhgfd"));
	}
}
